...
def build_game(matrix, i, j, number):
    ...
    if check(matrix, i, j, number):
        ...
        for _number in shuffle_number(numberlist):
            _matrix=build_game(_matrix, next_i, next_j, _number)
            if _matrix and sum([sum(row) for row in _matrix])==(sum(range(1,10))*9):
                return _matrix
    return matrix

def give_me_a_game(blank_size=9):
    matrix_all=build_game(matrix, 0, 0, random.choice(numberlist))
    set_ij=set()
    while len(list(set_ij))<blank_size:
        set_ij.add(str(random.choice(numberlist)-1)+','+str(random.choice(numberlist)-1))
